...
def save_models(folder):
	for i in range(gLetterCaseNum):
		fn = open(folder+ str(i) + "_a.model", "w")
		for ai in gv.models[i].a:
			fn.write(str(ai))
			fn.write('\n')
		fn.close()

		fn = open(folder + str(i) + "_b.model", "w")
		fn.write(str(gv.models[i].b))
		fn.close()

		fn = open(folder+ str(i) + "_y.model", "w")
		for j in range(len(gv.samples)):
...
def load_models():
	for i in range(gLetterCaseNum):
		fn = open(folder + str(i) + "_a.model", "r")
		j = 0
		for line in fn:
			gv.models[i].a[j] = float(line)
			j += 1
		fn.close()
		fn = open(folder + str(i) + "_b.model", "r")
...
def main(argv):
	...
	gv.RBF_dlt			= getInputArgv('-dlt', argv,1, None)
	optUseLinearKernel 	= getInputArgv('-k', argv,  1, None)
	folder				= getInputArgv('-f', argv,  1, None)

	if traningX==None or traningY==None or C==None or gv.RBF_dlt==None or optUseLinearKernel==None or folder==None:
		print("Usage: py -x traningX -y traningY -c C -dlt RBFDelta -k useLinearKernel -f folder-for-save-parameter")
		return
	...
	for i in range(gLetterCaseNum):
		update_samples_label(i)
	testY = test()
	predY = numpy.array(testY).reshape(len(testY),1)
